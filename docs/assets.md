...
| `nodes.png` | 20 icônes 16 × 16 : les 11 de la carte (fouille, filon, bloc, galerie, carnet, pépite, campement, noyau, indice, vie, risque) puis les 9 marchandises du comptoir (ordre de `enum ShopItem`) ; choisies dans la planche Pixel Lab `assets/pixelab.png` par la table de `tools/import_icons.py` | palette libre (≤ 15 couleurs) ; les états de la carte (atteignable, passé, lointain) sont des variantes en gris calculées ; les 11 premières vont aussi en mémoire sprite (barre du haut des salles) |
