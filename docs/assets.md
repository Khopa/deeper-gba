...
| `marks.png` | métatuiles 16 × 16 : vide, croix, fouille, alerte, gemme, minerai clair, minerai sombre, fantômes, éclats, puis nombres 1–12 | palette libre (≤ 15 couleurs + transparent), prise telle quelle par le moteur. **Dessiner la bande entière dans `assets/high-res/marks.png`** (magenta ou alpha = transparent, autant de couleurs qu'on veut : l'import la ramène à 15) ; fouille, gemme et les deux minerais sont ensuite pris dans la planche Pixel Lab (`import_icons.py --marks`) ; dans les filons, les deux minerais sont remplacés par deux gemmes de `nodes/` tirées selon la profondeur ; sinon des marques isolées dans `assets/high-res/marks/<nom>.png` remplacent celles de la planche de concept |
| `nodes/<nom>.png` | une icône 14 ou 16 px par fichier (nœuds de la carte, marchandises, gemmes des filons, minerai, clef…) ; `tools/import_icons.py` les lit dans l'ordre de sa table `ICONS` (= `enum Icon`) et écrit directement `build/gen/gfx_icons.c/.h` | **une palette par icône** (quantifiée à 15 couleurs si besoin) : le jeu charge la palette de l'icône qu'il dessine dans une banque libre (carte : banques 1–4 pour les nœuds atteignables, comptoir : une par ligne ; sprites : banques OBJ 8–15, un emplacement chacun) ; les nœuds lointains / passés utilisent une version en gris (rampe de luminance, banques 7–8) |
| `cursor.png` | sprite 16 × 16, 2 images | 1 = blanc |
| `dwarf.png` | 20 images 64 × 64 (l'art de 48 px centré) : repos (9), marche (9), de face, de dos ; issues de `assets/dwarf/dwarf-idle.png` et `dwarf-walking.png` (grilles 3 × 3 de 48 px), `dwarf-48.png` et `dwarf-back.png` par `tools/import_anim.py` (une seule palette pour les quatre) | palette libre (15 couleurs) ; banque OBJ 1 ; miroir dans les salles (il regarde la grille), moitié de taille sur la carte (matrice affine 3). Une seule image à la fois en VRAM (tuiles 64–127 de la zone partagée avec le marchand, le logo et les monstres), rechargée quand la boucle avance |
